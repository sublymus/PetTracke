...
export async function removeNotifContext(data: {
 ...
 async function addNotifContext(data: {
  context_name:string, 
  context_id: string,
  user: UserInterface
}) {
 ...
 async function getUserBrowser(data: {
  user_id?: string,
  user_agent?: string,
  user_browser_id?: string,
  enable?: boolean,
 ...
 async function removeUserBrowser(data: {
  user_browser_id: string,
  user: UserInterface
}) {
  const headers = new Headers();
 ...
 function testNotifier() {
  if ('serviceWorker' in navigator) {
    _send().catch(err => console.error(err));
  }
}
...
const NotifContext = {
  ...
  add:addNotifContext,
  remove:removeNotifContext,
  get:get_notif_contexts,
  required:requiredNotification,
  sendData:sendNotificationData,
  testNotifier,
  UserBrowser:{
    get:getUserBrowser,
    remove:removeUserBrowser,
  },
